...
export class DashboardComponent implements OnInit {
  ...
  getVolumePercent(volume: number): number {
    if (!this.dashboard || this.dashboard.muscleGroupBreakdown.length === 0) return 0;
    const max = Math.max(...this.dashboard.muscleGroupBreakdown.map(m => m.totalVolume));
  ...
  private normalizeDashboard(data: DashboardSummary | null | undefined): DashboardSummary {
    const fallback = this.getEmptyDashboard();

    return {
      totalWorkouts: data?.totalWorkouts ?? fallback.totalWorkouts,
      trainingStreakDays: data?.trainingStreakDays ?? fallback.trainingStreakDays,
      totalVolumeAllTime: data?.totalVolumeAllTime ?? fallback.totalVolumeAllTime,
      currentWeight: data?.currentWeight ?? fallback.currentWeight,
      currentBMI: data?.currentBMI ?? fallback.currentBMI,
      topPersonalRecords: Array.isArray(data?.topPersonalRecords) ? data!.topPersonalRecords : [],
      muscleGroupBreakdown: Array.isArray(data?.muscleGroupBreakdown) ? data!.muscleGroupBreakdown : []
    };
  }
